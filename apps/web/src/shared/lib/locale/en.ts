export const en = {
	"add": "Add",
	"addContent": "Add content",
	"addFirstContent": "Add your first note, document, or media file.",
	"addContent.eyebrow": "Add content",
	"addContent.title": "What do you want to add?",
	"addTag": "+ Add tag",
	"addContent.description": "Choose a content type first, then only the relevant fields and actions appear.",
	"audio": "Audio",
	"appearance": "Appearance",
	"appearance.title": "Appearance",
	"appearance.description": "Choose how Synapse looks on this device.",
	"appearance.theme.title": "Theme",
	"appearance.theme.system": "System",
	"appearance.theme.light": "Light",
	"appearance.theme.dark": "Dark",
	"appearance.palette.title": "Color palette",
	"appearance.palette.description": "Sets the accent character while keeping surfaces calm and familiar.",
	"appearance.palette.desert": "🏜️ Desert",
	"appearance.palette.twilight": "🌌 Twilight",
	"appearance.palette.arctic": "❄️ Arctic",
	"appearance.palette.noir": "⚫ Noir",
	"appearance.palette.forest": "🌲 Forest",
	"appearance.palette.ember": "🌋 Ember",
	"appearance.palette.slate": "🪨 Slate",
	"appearance.palette.sakura": "🌸 Sakura",
	"appearance.noteSparkles.title": "Note ambience",
	"appearance.noteSparkles.description":
		"Shows a subtle pixel glow beside a note after it has been open for a while.",
	"appearance.tagColors.title": "Automatic tag colors",
	"appearance.tagColors.description": "Assign a palette color whenever a new tag is created.",
	"autoplay.description": "Automatically starts audio and video when opened in the viewer.",
	"autoplay.title": "Autoplay",
	"aiUsage": "AI usage",
	"aiUsage.eyebrow": "AI workspace",
	"aiUsage.planDescription": "Your current monthly AI budget and activity.",
	"aiUsage.thisMonth": "Monthly limit for {month}",
	"aiUsage.limits": "Monthly limits",
	"aiUsage.tokens": "Tokens",
	"aiUsage.requests": "Requests",
	"aiUsage.successRate": "Success rate",
	"aiUsage.cost": "Estimated cost",
	"aiUsage.latency": "Avg. response",
	"aiUsage.failures": "Failed requests",
	"aiUsage.models": "Models used",
	"aiUsage.tokensShort": "tokens",
	"aiUsage.noRequests": "No requests yet",
	"aiUsage.empty": "AI usage will appear here after the first tag suggestion.",
	"aiUsage.error": "AI usage is temporarily unavailable.",
	"editor.bold": "Bold",
	"editor.italic": "Italic",
	"editor.underline": "Underline",
	"editor.strike": "Strike",
	"editor.code": "Inline code",
	"editor.link": "Link",
	"editor.bulletList": "Bullet list",
	"editor.orderedList": "Ordered list",
	"editor.taskList": "Task list",
	"editor.blockquote": "Blockquote",
	"editor.codeBlock": "Code block",
	"editor.separator": "Separator",
	"editor.image": "Image",
	"editor.commandsNotFound": "No commands found",
	"editor.heading2": "Heading 2",
	"editor.heading3": "Heading 3",
	"editor.heading4": "Heading 4",
	"editor.blockType": "Block type",
	"editor.paragraph": "Text",
	"editor.placeholder": "Start with a thought, idea, or observation…",
	"editor.noteContent": "Note content",
	"editor.imageLoadError": "Failed to add image",
	"editor.linkPrompt": "Link address",
	"cancel": "Cancel",
	"clearFilters": "Clear filters",
	"contentType.audio.description": "Files, tracks, and voice materials",
	"contentType.doc.description": "PDF, DOCX, EPUB, XLSX, CSV, and other documents",
	"contentType.link.description": "Saved links with previews and metadata",
	"contentType.media.description": "Images and videos for quick browsing",
	"contentType.note.description": "Quick thoughts, notes, and long-form text",
	"contentType.todo.description": "Short todo lists and checkpoints",
	"createdWithUs": "Member since {date}",
	"dashboard.drop.title": "Drop files to add content",
	"dashboard.drop.subtitle": "Images, video, audio, and documents are supported",
	"details": "Details",
	"delete": "Delete",
	"done": "done",
	"documents": "Document",
	"edit": "Edit",
	"emptyNote": "Empty note",
	"empty.description": "Add your first note, document, or media file.",
	"empty.title": "Nothing here yet",
	"files": "Files",
	"filter.types.active": "Content type filters are active",
	"filter.types.show": "Show content type filters",
	"general": "General",
	"generateTags": "AI tags",
	"generatingTags": "Generating…",
	"graph": "Graph",
	"home": "Home",
	"language": "Interface language",
	"language.description": "Choose the language for labels, menus, and main app screens.",
	"language.english": "English",
	"language.russian": "Русский",
	"link": "Link",
	"media": "Media",
	"mediaStorage": "Storage",
	"noDate": "Date unavailable",
	"notFound.description": "Change the query or clear filters.",
	"notFound.title": "Nothing found",
	"note": "Note",
	"open": "Open",
	"undo": "Undo",
	"repeat": "Repeat",
	"position.collapse": "Collapse",
	"position.expand": "Expand",
	"save": "Save",
	"saving": "Saving...",
	"search.aria": "Search content",
	"search.placeholder": "Search by title and content",
	"settings": "Settings",
	"settings.close": "Close settings",
	"settings.sidebar.collapse": "Collapse sidebar",
	"settings.sidebar.expand": "Expand sidebar",
	"settings.title": "Settings",
	"session.description": "End work on this device.",
	"session.signingOut": "Signing out…",
	"session.signOut": "Sign out",
	"session.title": "Current session",
	"account.delete.title": "Delete account",
	"account.delete.description": "All content and settings will be permanently removed.",
	"account.delete.action": "Delete account",
	"account.delete.confirmTitle": "Delete account?",
	"account.delete.confirmDescription": "This action cannot be undone.",
	"account.delete.confirm": "Delete permanently",
	"storage.local": "Local storage",
	"storage.used": "Used",
	"tags": "Tags",
	"tagColor.none": "No color",
	"tagColor.option": "Color {number}",
	"tagColor.picker": "Tag color",
	"todo": "Tasks",
	"untitled": "Untitled",
	"view.fullscreen": "Full screen",
	"view.windowed": "Collapse",
	"viewer.addTag": "Add tag...",
	"viewer.artist": "Artist: {artist}",
	"viewer.author": "Author: {author}",
	"viewer.close": "Close viewer",
	"viewer.contentDeleted": "Content deleted",
	"viewer.created": "Created {date}",
	"viewer.coverAlt": "Cover",
	"viewer.deleteError": "Failed to delete content",
	"viewer.deleteTitle": "Delete content?",
	"viewer.deleteDescription": "This action cannot be undone. The item will be deleted permanently.",
	"viewer.documentPreviewAlt": "Document preview",
	"viewer.download": "Download",
	"viewer.downloadError": "Failed to download file",
	"viewer.downloading": "Downloading...",
	"viewer.emptyTasks": "No tasks",
	"viewer.loading": "Loading...",
	"viewer.loadingNote": "Loading the full note",
	"viewer.mediaAlt": "Media",
	"viewer.lessThanMinute": "less than a minute",
	"viewer.minutes": "min",
	"viewer.hour": "h",
	"viewer.hours": "h",
	"viewer.next": "Next item",
	"viewer.noCover": "No cover",
	"viewer.previous": "Previous item",
	"viewer.tagAdded": "Tag added",
	"viewer.tagAddError": "Failed to add tag",
	"viewer.tagRemoved": "Tag removed",
	"viewer.tagRemoveError": "Failed to remove tag",
	"viewer.tagsError": "Failed to add tags",
	"viewer.updated": "Updated {date}",
} as const;

export const searchPlaceholders = [
	"Search your notes, links, and ideas",
	"What are we looking for today?",
	"Find that thought hiding somewhere",
	"Search the whole Synapse",
	"Where did I put that brilliant idea?",
	"Look for a note, link, or tiny revelation",
	"Try a title, tag, or phrase",
	"Search like you mean it",
	"Hunt down a half-remembered thought",
	"Find the thread worth pulling",
	"What should we bring back to the surface?",
	"Search your second brain",
	"Type a clue. We’ll do the digging.",
	"Find something you saved for later",
	"Browse your memory palace",
	"Looking for a spark?",
	"Search by title, tag, or content",
	"Let’s find that little gem",
	"Which idea is calling?",
	"Start with a word or two",
] as const;

export type TranslationKey = keyof typeof en;
export type TranslationMap = Record<TranslationKey, string>;
