...
interface SliderEngineProps extends Omit<HTMLAttributes<HTMLDivElement>, "onChange" | "defaultValue"> {
	value: SliderValue;
...
function TooltipValue({ value, formatValue, motionX }: TooltipValueProps) {
	...
	return (
		<motion.div
			...
			<span
				className={cn(
					"bg-foreground px-2 py-1 text-[12px] whitespace-nowrap text-background tabular-nums",
					shape.bg
				)}
				style={{ fontVariationSettings: fontWeights.medium }}>
				{formatValue(value)}
				...
				<div
					...
					<div
						...
						<motion.div
							...
							{!hideFill && (
								<motion.div
									className={cn("bg-selected/50 absolute h-full dark:bg-accent/40", fillClassName)}
									style={{
										left: fillLeft,
										width: fillWidth,
										...fillStyle,
		...
		return (
			<div
				...
				<motion.div
					ref={mergedRef}
					className={cn(
						"relative h-8 w-full touch-none overflow-hidden border border-border outline-offset-2 select-none",
					...
					{variant === "pips" && (
						<div className="pointer-events-none absolute inset-0 z-[2] flex items-center px-2" aria-hidden>
							{label && (
								<span className="bg-background px-2 text-[13px] text-transparent select-none">{label}</span>
							)}
							<span
...
const Slider = forwardRef<HTMLDivElement, SliderProps>(({ size, variant = "pips", ...props }, ref) => {
